...
def extract_sudoku_grid(input_path, output_path, contour_path, cell_dir, img_name):
    ...
    canny_path = os.path.join(MEDIA_DIR, f'canny_{img_name}.png')
    cv2.imwrite(canny_path, edges)

    # All Contours
    contours, _ = cv2.findContours(edges, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    imgAllContours = image.copy()
    cv2.drawContours(imgAllContours, contours, -1, (0, 255, 0), 10)
    allContours_path = os.path.join(MEDIA_DIR, f'allContours_{img_name}.png')
    cv2.imwrite(allContours_path, imgAllContours)
    ...
    for contour in contours:
        # Use the convex hull to avoid cutting corners
        hull = cv2.convexHull(contour)

        # Approximate the hull to reduce points
        epsilon = 0.02 * cv2.arcLength(hull, True)
        approx = cv2.approxPolyDP(hull, epsilon, True)

        # Check for a 4-sided polygon
        if len(approx) == 4:
            # Use the hull area instead of the contour area for more accurate filtering
            ...
            # More flexible aspect ratio check for curved grids
...
def extract_sudoku_grid_canny(input_path, output_path, contour_path, cell_dir, img_name):
    image = cv2.imread(input_path)
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    blurred = cv2.GaussianBlur(gray, (5, 5), 0)
...
def extract_sudoku_grid_back(input_path, output_path, contour_path, cell_dir, img_name):
    # Read the uploaded image
    image = cv2.imread(input_path)
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    blurred = cv2.GaussianBlur(gray, (5, 5), 0)
